...
static void
lldp_setup_chassis(struct lldp *lldp)
{
	struct ifreq ifr;

	memset(&ifr, 0, sizeof(ifr));
	ifr.ifr_addr.sa_family = AF_INET;
	strncpy(ifr.ifr_name, lldp->ifname, IFNAMSIZ - 1);
	if (!ioctl(lldp->fd.fd, SIOCGIFADDR, &ifr))
		memcpy(lldp_global.mgmt_ipv4, &((struct sockaddr_in *)&ifr.ifr_addr)->sin_addr, 4);
	memcpy(lldp_global.addr, ifr.ifr_hwaddr.sa_data, ETH_ALEN);

	ULOG_INFO("using %s for the chassis-id and mgmt port\n", lldp->ifname);
}

static int
lldp_open(struct lldp *lldp, char *ifname)
{
	__u8 chassis[] = { 0x0, 0x0, 0x0, 0x0, 0x0, 0x0 };
	...
	if (!memcmp(lldp_global.addr, chassis, ETH_ALEN))
		lldp_setup_chassis(lldp);
